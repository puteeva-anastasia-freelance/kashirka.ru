(function () {
	...
	/**
	 * Класс для отрисовки баннера участка
	 */
	class BannerPlot {
		constructor() {
		...
		/**
		 * Метод вставляет баннер участка на страницу
		 * @param {PlotDTO} plot объект с информацией об участке
		 * @param {TownshipDTO[]} townships массив поселков из файла townships.js
		 */
		insertBannerIntoPage(plot, townships) {
			this.checkPlotExists(plot);

			let bannerTopMarkup = this.getBannerTopMarkup(plot, townships);
			this.bannerWrapEl.insertAdjacentHTML('beforeend', bannerTopMarkup); 

			let bannerBottomMarkup = this.getBannerBottomMarkup(plot, townships);
		...
		/**
		 * Метод проверяет существует ли участок, если не существует, то появляется страница 404
		 * @param {PlotDTO} plot объект с информацией об участке
		 */
		checkPlotExists(plot) {
			if (!plot) {
		...
		/**
		 * Метод получает коммуникации поселка
		 * @param {PlotDTO} plot объект с информацией об участке
		 * @param {TownshipDTO[]} townships массив поселков из файла townships.js
		 * @returns {array} коммуникации поселка
		 */
		getCommunicationsTownship(plot, townships) {
			for (let township of townships) {
				if (plot.townshipId == township.id) {
					return township.communications;
				}
			}
		}

		/**
		 * Метод получает удаленность поселка от МКАД
		 * @param {PlotDTO} plot объект с информацией об участке
		 * @param {TownshipDTO[]} townships массив поселков из файла townships.js
		 * @returns {number} удаленность поселка от МКАД
		 */
		getDistanceTownship(plot, townships) {
			for (let township of townships) {
				if (plot.townshipId == township.id) {
					return township.distance;
				}
			}
		}

		/**
		 * Метод получает название поселка, в котором расположен данный участок
		 * @param {PlotDTO} plot объект с информацией об участке
		 * @param {TownshipDTO[]} townships массив поселков из файла townships.js
		 * @returns {string} название поселка, в котором расположен данный участок
		 */
		getNameTownship(plot, townships) {
			for (let township of townships) {
				if (plot.townshipId == township.id) {
		...
		/**
		 * Этот метод генерирует разметку верхней части баннера участка
		 * @param {PlotDTO} plot объект с информацией об участке
		 * @param {TownshipDTO[]} townships массив поселков из файла townships.js
		 * @returns {string} html-разметка верхней части баннера участка
		 */
		getBannerTopMarkup(plot, townships) {
			let nameTownship = this.getNameTownship(plot, townships);

			return `<h2 class="h1 h1-title banner__title">Участок&nbsp;${plot.area} соток в&nbsp;КП&nbsp;${nameTownship}</h2>
		...
		/**
		 * Этот метод генерирует разметку нижней части баннера участка
		 * @param {PlotDTO} plot объект с информацией об участке
		 * @param {TownshipDTO[]} townships массив поселков из файла townships.js
		 * @returns {string} html-разметка нижней части баннера участка
		 */
		getBannerBottomMarkup(plot, townships) {
			let distanceTownship = this.getDistanceTownship(plot, townships);
			let communicationsTownship = this.getCommunicationsTownship(plot, townships);
			let commMarkup = this.getCommMarkup(communicationsTownship);

			return `<div class="container banner__bottom-container">
			<div class="banner__col">
				<span class="banner__subtitle">от МКАД:</span>
				<span class="banner__count"><span class="banner__count banner__count_big">${distanceTownship} </span>км.</span>
			</div>
			<div class="banner__col">
				<span class="banner__subtitle">Стоимость участка:</span>
				<span class="banner__count"><span class="banner__count banner__count_big">${this.formatNumber(plot.price)} </span>руб.</span>
			</div>
			<div class="banner__col">
				<span class="banner__subtitle">Номер на плане:</span>
				<span class="banner__count"><span class="banner__count banner__count_big">${plot.numberOnPlan}</span></span>
			</div>
			${commMarkup}
		</div>`;
		}
		...
		/**
		 * Метод получает разметку коммуникаций
		 * @param {TownshipDTO} communicationsTownship коммуникации поселка
		 * @returns {string} html-разметка коммуникаций
		 */
		getCommMarkup(communicationsTownship) {
			let commMarkup = '';

			for (let i = 0; i < communicationsTownship.length; i++) {
				switch (communicationsTownship[i]) {
					case 'elec':
	...
	window.addEventListener('load', () => {
		let bannerPlot = new BannerPlot();
		let idPlot = location.hash.replace(/#/, '');
		let id = plots.findIndex(x => x.id == idPlot);
		bannerPlot.insertBannerIntoPage(plots[id], townships);
		bannerPlot.reloadPage();
	});
})();