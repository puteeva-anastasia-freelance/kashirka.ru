(function () {
	...
	/**
	 * Класс для отрисовки баннера поселка
	 */
	class BannerTownship {
		constructor() {
			this.pathToTownshipsImages = 'assets/img/dist/townships';

			this.bannerWrapEl = document.querySelector('.banner__wrap');
			this.bannerTopEl = document.querySelector('.banner__top');
			this.bannerBottomEl = document.querySelector('.banner__bottom');
			this.likedCountEl = document.querySelector('.main-header__liked-count');
		}

		/**
		 * Метод вставляет баннер поселка на страницу
		 ...
		 */
		insertBannerIntoPage(township, plots, idTownship) {
			this.checkTownshipExists(township);

			let bannerTopMarkup = this.getBannerTopMarkup(township);
			let bannerBottomMarkup = this.getBannerBottomMarkup(township, plots, idTownship);
			this.bannerWrapEl.insertAdjacentHTML('beforeend', bannerTopMarkup);
			this.bannerBottomEl.insertAdjacentHTML('beforeend', bannerBottomMarkup);
			this.setBackgroundImageTownship(township);
			this.addLikedClickListener();
		}

		/**
		 * Метод проверяет существует ли поселок, если не существует, то появляется страница 404
		 * @param {TownshipDTO} township объект с информацией о поселке
		 */
		checkTownshipExists(township) {
			if (!township) {
		...
		/**
		 * Метод получает количество участков в поселке, выставленных на продажу
		 * @param {PlotDTO[]} plots массив участков из файла plots.js
		 * @param {number} idTownship идентификатор поселка
		 * @returns {number} количество участков в поселке, выставленных на продажу
		 */
		getNumberPlotsForSale(plots, idTownship) {
			let count = 0;
		...
		/**
		 * Метод добавляет лайку слушатель события клика
		 */
		addLikedClickListener() {
			let bannerLikedEl = document.querySelector('.banner__liked');
			bannerLikedEl.addEventListener('click', () => {
				let likedIconEl = bannerLikedEl.querySelector('.liked__icon');
		...
		/**
		 * Этот метод генерирует разметку верхней части баннера поселка
		 * @param {TownshipDTO} township объект с информацией о поселке
		 * @returns {string} html-разметка верхней части баннера поселка
		 */
		getBannerTopMarkup(township) {
			return `<h2 class="h1 h1-title banner__title">Коттеджный поселок ${township.name}</h2>
			<span class="banner__txt">Каширское шоссе, Домодедовский район</span>
			<div class="banner__inner">
				<button class="button banner__button">Каталог участков</button> 
		...
		/**
		 * Этот метод генерирует разметку нижней части баннера поселка
		 * @param {TownshipDTO} township объект с информацией о поселке
		 * @param {PlotDTO[]} plots массив участков из файла plots.js
		 * @param {number} idTownship идентификатор поселка
		 * @returns {string} html-разметка нижней части баннера поселка
		 */
		getBannerBottomMarkup(township, plots, idTownship) {
			let commMarkup = this.getCommTownshipMarkup(township);
			let minCostHundred = this.getMinCostHundred(plots, idTownship);
			let numberPlotsForSale = this.getNumberPlotsForSale(plots, idTownship);

			return `<div class="container banner__bottom-container">
			<div class="banner__col">
				<span class="banner__subtitle">от МКАД:</span>
				<span class="banner__count"><span class="banner__count banner__count_big">${township.distance} </span>км.</span>
			</div>
			<div class="banner__col">
				<span class="banner__subtitle">Стоимость сотки:</span>
				<span class="banner__count">от<span class="banner__count banner__count_big"> ${this.formatNumber(minCostHundred)} </span>руб.</span>
			</div>
			<div class="banner__col">
				<span class="banner__subtitle">Участков в продаже:</span>
				<span class="banner__count"><span class="banner__count banner__count_big">${numberPlotsForSale}</span></span>
			</div>
			${commMarkup}
		</div>`;
		}
		...
		/**
		 * Метод получает разметку коммуникаций поселка
		 * @param {TownshipDTO} township объект с информацией о поселке
		 * @returns {string} html-разметка коммуникаций поселка
		 */
		getCommTownshipMarkup(township) {
			let commMarkup = '';

			for (let i = 0; i < township.communications.length; i++) {
				switch (township.communications[i]) {
					case 'elec':
						commMarkup += `<div class="banner__comm-item">
		...
		/**
		 * Метод устанавливает фоновую картинку поселка
		 * @param {TownshipDTO} township объект с информацией о поселке
		 */
		setBackgroundImageTownship(township) {
			if (window.innerWidth <= 576) {
				this.bannerTopEl.style.background = `center / cover no-repeat url(${this.pathToTownshipsImages}/${township.imageMiddle})`;
			} else {
				this.bannerTopEl.style.background = `center / cover no-repeat url(${this.pathToTownshipsImages}/${township.image})`;
			}
	...
	window.addEventListener('load', () => {
		let bannerTownship = new BannerTownship();
		let idTownship = location.hash.replace(/#/, '');
		let id = townships.findIndex(x => x.id == idTownship);
		bannerTownship.insertBannerIntoPage(townships[id], plots, idTownship);
	});
})();