(function () {
	...
	/**
	 * Класс для отрисовки карточек новостей "Читайте также"
	 */
	class NewsReadAlso {
		constructor() {
		...
		getWindowWidth(news) {
			...
			this.insertNewsIntoPage(numberOfCards, news);
		 ...
		 * @param {NewsDTO[]} news массив новостей из файла news.js
		 */
		insertNewsIntoPage(numberOfCards, news) {
			let count = 0;
			let newsMarkup = '';
			...
			for (let item of news) {
				...
				if (count < numberOfCards && idArticle != item.id) {
	...
	window.addEventListener('load', () => {
		let newsReadAlso = new NewsReadAlso();
		newsReadAlso.getWindowWidth(news);
	});
})();