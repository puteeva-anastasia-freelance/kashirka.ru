!function(){"use strict";class s{constructor(){this.pathToTownshipsImages="assets/img/dist/townships",this.bannerWrapEl=document.querySelector(".banner__wrap"),this.bannerTopEl=document.querySelector(".banner__top"),this.bannerBottomEl=document.querySelector(".banner__bottom"),this.likedCountEl=document.querySelector(".main-header__liked-count")}insertBannerIntoPage(n,t,e){this.checkTownshipExists(n);var s=this.getBannerTopMarkup(n),t=this.getBannerBottomMarkup(n,t,e);this.bannerWrapEl.insertAdjacentHTML("beforeend",s),this.bannerBottomEl.insertAdjacentHTML("beforeend",t),this.setBackgroundImageTownship(n),this.addLikedClickListener()}checkTownshipExists(n){n||(window.location.href="404.html")}getNumberPlotsForSale(n,t){let e=0;return n.forEach(n=>{n.townshipId==t&&e++}),e}getMinCostHundred(t,e){var s=[];let a=0;for(let n=0;n<t.length;n++)t[n].townshipId==e&&(s[a]=(t[n].price/t[n].area).toFixed(0),a++);return Math.min(...s)}addLikedClickListener(){let t=document.querySelector(".banner__liked");t.addEventListener("click",()=>{var n=t.querySelector(".liked__icon");n.classList.contains("active")?(n.classList.remove("active"),this.likedCountEl.dataset.quantity=+this.likedCountEl.dataset.quantity-1):(n.classList.add("active"),this.likedCountEl.dataset.quantity=+this.likedCountEl.dataset.quantity+1),this.likedCountEl.textContent=this.likedCountEl.dataset.quantity})}getBannerTopMarkup(n){return`<h2 class="h1 h1-title banner__title">Коттеджный поселок ${n.name}</h2>
			...
			</div>`}formatNumber(n){return n.toString().replace(/(\d)(?=(\d\d\d)+([^\d]|$))/g,"$1 ")}getBannerBottomMarkup(n,t,e){var s=this.getCommTownshipMarkup(n),a=this.getMinCostHundred(t,e),t=this.getNumberPlotsForSale(t,e);return`<div class="container banner__bottom-container">
			<div class="banner__col">
				<span class="banner__subtitle">от МКАД:</span>
				<span class="banner__count"><span class="banner__count banner__count_big">${n.distance} </span>км.</span>
			</div>
			<div class="banner__col">
				<span class="banner__subtitle">Стоимость сотки:</span>
				<span class="banner__count">от<span class="banner__count banner__count_big"> ${this.formatNumber(a)} </span>руб.</span>
			</div>
			<div class="banner__col">
				<span class="banner__subtitle">Участков в продаже:</span>
				<span class="banner__count"><span class="banner__count banner__count_big">${t}</span></span>
			</div>
			${s}
		</div>`}getCommTownshipMarkup(t){let e="";for(let n=0;n<t.communications.length;n++)switch(t.communications[n]){case"elec":e+=`<div class="banner__comm-item">
			...
			</div>`}setBackgroundImageTownship(n){window.innerWidth<=576?this.bannerTopEl.style.background=`center / cover no-repeat url(${this.pathToTownshipsImages}/${n.imageMiddle})`:this.bannerTopEl.style.background=`center / cover no-repeat url(${this.pathToTownshipsImages}/${n.image})`}}window.addEventListener("load",()=>{var n=new s;let t=location.hash.replace(/#/,"");var e=townships.findIndex(n=>n.id==t);n.insertBannerIntoPage(townships[e],plots,t)})}();